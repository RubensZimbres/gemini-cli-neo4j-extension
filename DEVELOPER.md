...
and contributing to the Neo4j Gemini CLI Extension project.
...
2.  **Neo4j Instance:** For testing data plane tools, you will need access to an active Neo4j instance.
...
1.  **Clone the Repository:**
    ...
    ```bash
    git clone https://github.com/gemini-cli-extensions/neo4j.git
    cd neo4j
    ```
...
4.  **Testing Changes:** After installation, start the Gemini CLI (`gemini`).
    You can now interact with the `neo4j` tools to manually test your changes
...
metadata files (`gemini-extension.json`, `NEO4J.md`, `LICENSE`) along with the
...
*   `@gemini-cli-extensions/neo4j-maintainers`
...
3. **Filtering:** These release notes are filtered to include only changes
   ...
   `neo4j`), passed as an environment variable in the workflow file.
